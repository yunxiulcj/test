toggleRowSelection<template>
  <div class="table-temp" ref="tableTempWrap">
    <el-table
      ...
      :border="cloneConfig.border || true"
      :row-key="cloneConfig.rowKey"
      lazy
      :default-sort="cloneConfig.defaultSort"
      :load="cloneConfig.loadFn"
      :stripe="
        typeof cloneConfig.stripe == 'undefined' ? true : cloneConfig.stripe
      "
      size="small"
      @row-click="rowClick"
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <slot name="btn" :row="scope.row">
            <el-button
              v-for="(item, index) in cloneConfig.operation.btns"
              :key="index"
              v-show="
                typeof item.show === 'function'
                  ? item.show(scope.row)
    ...
    <div class="report-nav" v-if="cloneConfig.page" ref="reportNav">
      <el-pagination
...
export default {
  ...
  methods: {
    ...
    toggleRowSelection(data) {
      ...
      this.$nextTick(() => {
        this.$refs['elTable'].toggleRowSelection(data, true)
      })
    },
  },
